...
namespace FSF.CollectionFrame
{
    public class Lyric_Demo : MonoBehaviour
    {
        ...
        [HideInInspector] public MovementDirection movementDirection = MovementDirection.Left;
        [HideInInspector] public float movementDistance = 100f;
        [HideInInspector] public AnimationCurve animationCurve = AnimationCurve.EaseInOut(0, 0, 1, 1);
        ...
        private void Start()
        {
            ...
            {
                Keys = LyricSpliter.Split(LyricFile.text);
                keyCount = Keys.Count;
                currentIndex = 0;
                if (keyCount > 0)
                {
    ...
    public class LyricDemoEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            base.OnInspectorGUI();
            var T = target as Lyric_Demo;
            if (T.enableMovementEffect)
            {
                T.movementDirection = (Lyric_Demo.MovementDirection)EditorGUILayout.EnumPopup("Movement Direction", T.movementDirection);
                T.movementDistance = EditorGUILayout.FloatField("Movement Distance", T.movementDistance);
                T.animationCurve = EditorGUILayout.CurveField("Animation Curve", T.animationCurve);
            }
        }
    }
#endif
